...
mv /var/lib/pbench-agent/pbench-user-benchmark* /var/www/html/pub/radosWrite.$k.$m && \
echo "Data Writen into the pool" && \
clean_ceph &&
sleep 5

pbench-user-benchmark "./radosbench.sh -o read -k $k -m $m -t $run_time -p $thread  -b $b_size" && \
mv /var/lib/pbench-agent/pbench-user-benchmark* /var/www/html/pub/radosRead.$k.$m && \
cleanup
}
